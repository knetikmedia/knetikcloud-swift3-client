...
open class DevicesAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func updateDevice(device: DeviceResource, id: Int32, completion: @escaping ((_ error: Error?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    open class func updateDeviceWithRequestBuilder(device: DeviceResource, id: Int32) -> RequestBuilder<Void> {
        var path = "/devices/{id}"
        path = path.replacingOccurrences(of: "{id}", with: "\(id)", options: .literal, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = device.encodeToJSON() as? [String:AnyObject]
