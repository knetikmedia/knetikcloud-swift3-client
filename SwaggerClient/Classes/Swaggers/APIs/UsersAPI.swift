...
open class UsersAPI: APIBase {
  ...
  "username" : "aeiou"
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func updateUserTemplate(id: String, userTemplateResource: TemplateResource? = nil, completion: @escaping ((_ error: Error?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    open class func updateUserTemplateWithRequestBuilder(id: String, userTemplateResource: TemplateResource? = nil) -> RequestBuilder<Void> {
        var path = "/users/templates/{id}"
        path = path.replacingOccurrences(of: "{id}", with: "\(id)", options: .literal, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = userTemplateResource?.encodeToJSON() as? [String:AnyObject]
