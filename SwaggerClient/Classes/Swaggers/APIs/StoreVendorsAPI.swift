...
open class StoreVendorsAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func updateVendor(id: Int32, vendor: VendorResource? = nil, completion: @escaping ((_ error: Error?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    open class func updateVendorWithRequestBuilder(id: Int32, vendor: VendorResource? = nil) -> RequestBuilder<Void> {
        var path = "/vendors/{id}"
        path = path.replacingOccurrences(of: "{id}", with: "\(id)", options: .literal, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = vendor?.encodeToJSON() as? [String:AnyObject]
