...
open class BRERuleEngineTriggersAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func updateBRETrigger(eventName: String, breTriggerResource: BreTriggerResource? = nil, completion: @escaping ((_ error: Error?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    open class func updateBRETriggerWithRequestBuilder(eventName: String, breTriggerResource: BreTriggerResource? = nil) -> RequestBuilder<Void> {
        var path = "/bre/triggers/{eventName}"
        path = path.replacingOccurrences(of: "{eventName}", with: "\(eventName)", options: .literal, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = breTriggerResource?.encodeToJSON() as? [String:AnyObject]
