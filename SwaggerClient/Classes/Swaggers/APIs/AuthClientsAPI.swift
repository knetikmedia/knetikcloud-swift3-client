...
open class AuthClientsAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func updateClient(clientKey: String, clientResource: ClientResource? = nil, completion: @escaping ((_ error: Error?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    open class func updateClientWithRequestBuilder(clientKey: String, clientResource: ClientResource? = nil) -> RequestBuilder<Void> {
        var path = "/auth/clients/{clientKey}"
        path = path.replacingOccurrences(of: "{clientKey}", with: "\(clientKey)", options: .literal, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = clientResource?.encodeToJSON() as? [String:AnyObject]
