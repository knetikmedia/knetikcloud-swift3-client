...
open class MediaArtistsAPI: APIBase {
  ...
  "first" : true
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func updateArtistTemplate(id: String, artistTemplateResource: TemplateResource? = nil, completion: @escaping ((_ error: Error?) -> Void)) {
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    open class func updateArtistTemplateWithRequestBuilder(id: String, artistTemplateResource: TemplateResource? = nil) -> RequestBuilder<Void> {
        var path = "/media/artists/templates/{id}"
        path = path.replacingOccurrences(of: "{id}", with: "\(id)", options: .literal, range: nil)
        let URLString = SwaggerClientAPI.basePath + path
        let parameters = artistTemplateResource?.encodeToJSON() as? [String:AnyObject]
